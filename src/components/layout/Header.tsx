    ...
    <>
      <header className="absolute top-0 left-0 w-full z-50 bg-white">
        <div className="flex items-center justify-between fluid-header">
          <Link
            href="/"
            className="relative block w-[clamp(2.5rem,11vw,5rem)] z-50"
          >
          ...
          <nav className="hidden md:flex items-center gap-[2vw] xl:gap-[2.5vw]">
            {NAV_ITEMS.map((item) => (
              ...
              >
                {item.label}
                <span className="absolute left-0 -bottom-1 h-px w-0 bg-brand-lime transition-all duration-300 group-hover:w-full"></span>
              </Link>
            ))}
          </nav>
      ...
      >
        <div className="flex flex-col h-full px-[clamp(1rem,2.5vw,2rem)] pt-[clamp(5rem,15vw,7rem)] pb-8">
          <nav className="flex flex-col gap-6 overflow-y-auto items-center no-scrollbar flex-1">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.href}
                href={item.href}
